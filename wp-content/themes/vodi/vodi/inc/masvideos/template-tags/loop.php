...
if ( ! function_exists( 'vodi_search_result_page_header' ) ) {
    ...
    function vodi_search_result_page_header() {
        if ( is_search() && vodi_is_masvideos_activated() && ( is_videos() || is_movies() || is_tv_shows() ) ) {
            $current_post_type_obj = get_queried_object();
            $post_type = $current_post_type_obj->name;
            ?>
            <header class="page__header stretch-full-width">
                <div class="container">
                    <h1 class="page__title"><?php call_user_func( "masvideos_{$post_type}_page_title" ); ?></h1>
        ...
        if( apply_filters( 'vodi_display_search_result_page_tab', true ) && ! empty( $search_query ) ) {
            ...
            if( $post_types > 1 ) {
                ...
                foreach ( $post_types as $key => $post_type ) {
                    ...
                    if( $current_post_type_obj->name == $post_type ) {
                        $class .= ' active';
                    }

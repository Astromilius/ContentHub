...
if ( ! function_exists( 'vodi_template_single_movie_action_buttons' ) ) {
    function vodi_template_single_movie_action_buttons() {
        global $movie;
        $ticket_button_text =  get_post_meta( $movie->get_ID(), '_vodi_movie_buy_ticket_text', true );
        $ticket_button_link = get_post_meta( $movie->get_ID(), '_vodi_movie_buy_ticket_link', true );
        $trailer_button_text = get_post_meta( $movie->get_ID(), '_vodi_movie_play_trailer_text', true );
        $trailer_button_link = get_post_meta( $movie->get_ID(), '_vodi_movie_play_trailer_link', true );
        $watch_button_text = __( 'Watch Now', 'vodi' );
        $ountput_content = '';
        ...
        if ( ( $movie->get_movie_choice() == 'movie_url' ) && $movie->get_movie_is_affiliate_link() ) {
                ...
                $ountput_content = ob_get_clean();
        } elseif( ! empty( masvideos_get_the_movie( $movie ) ) ) {
            ob_start();
            ?><a data-fancybox data-src="#hidden-movie-content" class="movie-actions--watch-now" href="javascript:;">
                <?php echo esc_html( $watch_button_text ); ?>
            </a>
            <div style="display: none;" class="single-movie-popup" id="hidden-movie-content">
        ...
        if ( ( ! empty( $ticket_button_text ) && ! empty( $ticket_button_link ) ) || ( ! empty( $trailer_button_text ) && ! empty( $trailer_button_link ) ) || ! empty( $ountput_content ) ) :
            ?>
            <div class="single-movie__actions">
                <?php if ( ! empty( $ountput_content ) ) {
                    echo $ountput_content;
                } ?>
